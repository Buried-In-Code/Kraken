...
class CharacterTest {
  ...
  inner class ListCharacters {
    ...
    fun `Test ListCharacters with a valid search`() {
      ...
      assertAll({ assertEquals(1234, results[0].id) }, { assertEquals("Smiley Bone", results[0].name) })
    }
  ...
  inner class GetCharacter {
    ...
    fun `Test GetCharacter with an invalid id`() {
      assertThrows(ServiceException::class.java) { session.getCharacter(id = -1) }
    }
    ...
    fun `Test GetCharacter with a null alias`() {
      ...
      assertNotNull(result)
      assertAll({ assertTrue(result.alias.isEmpty()) })
    }
    ...
    fun `Test GetCharacter with an alias`() {
      ...
      assertNotNull(result)
      assertAll({ assertEquals("Spy-D", result.alias[0]) })
    }
  }
}
